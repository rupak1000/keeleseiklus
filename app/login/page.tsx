...
export default function LoginPage() {
  ...
  // Re-introducing localStorage/sessionStorage helpers
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.error("Login API Error:", responseData.error || responseData.message);
      ...
      // Clear any existing user data first (important for clean session)
      localStorage.removeItem("user");
      localStorage.removeItem("isLoggedIn");
      sessionStorage.removeItem("user");
      sessionStorage.removeItem("isLoggedIn");

      // Store the received user data (excluding sensitive fields like password_hash)
      // The API is already selecting non-sensitive fields, so we can store responseData.user directly.
      const userSaved = saveToStorage("user", JSON.stringify(responseData.user));
      const loginSaved = saveToStorage("isLoggedIn", "true");

      if (userSaved && loginSaved) {
        console.log("✅ User data saved to client-side storage.");
        // Determine redirect path based on isAdmin flag from backend
        if (responseData.user.is_admin) {
          router.push("/admin"); // Redirect admin users
        } else {
          router.push("/modules"); // Redirect regular users
        }
      } else {
        setErrorMessage("Failed to save login data to browser storage. Please check browser settings.");
      }

    } catch (error: any) {
      console.error("Frontend Login Error:", error);
      setErrorMessage(error.message || "An unexpected error occurred during login. Please try again.");
    } finally {
      setIsSubmitting(false);
    }
  };

  // testStorage function remains for debugging browser storage capabilities
  const testStorage = () => {
          ...
          <CardContent>
            ...
            <div className="mt-6 p-3 bg-gradient-to-r from-gray-50 to-blue-50 rounded-lg border border-gray-200">
              <p className="text-xs text-gray-600 text-center">
                <strong>{t("login.adminAccess")}: rupakbd2011@gmail.com / 12356</strong>
              </p>
            </div>
