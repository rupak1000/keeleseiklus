...
export default function NewExamPage() {
  ...
  const duplicateQuestion = (sectionId: string, questionId: string) => {
    ...
    if (question) {
      ...
      addQuestion(sectionId, "multiple-choice")
      updateQuestion(sectionId, duplicatedQuestion.id, duplicatedQuestion)
    }
  }
  ...
  const saveExam = (publish = false) => {
    ...
    const existingTemplates = JSON.parse(localStorage.getItem("examTemplates") || "[]")
    const updatedTemplates = [...existingTemplates, updatedExam]
    localStorage.setItem("examTemplates", JSON.stringify(updatedTemplates))

    if (publish) {
      // Set as active exam if published
      localStorage.setItem("activeExamTemplate", JSON.stringify(updatedExam))
    }
        ...
        <div className="mb-8">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <Button variant="outline" onClick={() => saveExam(false)}>
